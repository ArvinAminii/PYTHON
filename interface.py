...
class Application:
    def __init__(self, master):
        self.master = master
        self.master.title("User Managment")
        self.master.geometry("880x350")
        ...
        # listboxs
        self.student_laybel = tk.Label(master, text="Students:")
        self.student_laybel.grid(row=0, column=3)
    ...
    def export_csv(self):
        ...
        if file_path_students:
            with open(file_path_students, "w", newline="", encoding="uft-8") as f:
                writer = csv.writer(f)
                writer.writerow(["Name", "Family", "Age", "Email"])
                for s in students:
                    writer.writerow(s)
                messagebox.showinfo(
                    "Success", "Students fata exported to csv file.")
        ...
        if file_path_teachers:
            with open(file_path_teachers, "w", newline="", encoding="uft-8") as f:
                writer = csv.writer(f)
                writer.writerow(["Name", "Family", "Age", "Email"])
                for t in teachers:
                    writer.writerow(t)
